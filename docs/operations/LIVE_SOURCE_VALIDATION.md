...
On an unchanged source, the second run should report `new=0` and `changed=0`; previously persisted calls should be `UNCHANGED` internally and no extra funding-call version should be created. Every attempt still gets its own `source_scan_runs` audit row.
